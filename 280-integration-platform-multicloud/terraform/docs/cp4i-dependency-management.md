# Cloud Pak for Integration Dependency Management
## Module overview

### Description

The IBM Cloud Paks are composed of many different software components, most if not all, are deployed using operators. Each component is installed from a different channel, using a different license key value, and using a different version. Independently, that is not an issue. However, many of these components have dependencies on each other and dependencies on particular versions. This module collects and manages the different interdependent versions for the components in Cloud Pak for Integration.
